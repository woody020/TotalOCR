...
class Trainer:
    def __init__(self, model, optimizer, lr_schedule, is_use_cuda, train_data_loader,
                 valid_data_loader, start_epoch=0, num_epochs=25, logger=None, model_name=None):
        self.model = model
    ...
    def _save_best_model(self):
        ...
        if not os.path.isdir('./checkpoint/' + self.model_name):
        ...
        torch.save(state,
                   './checkpoint/' + self.model_name + '/Models' + '_epoch_%d' % self.cur_epoch + '.ckpt')  # Notice
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for inputs_1, single_image_label_scans, single_image_label_sizes, single_image_label_types, single_image_label_emphas in self.dataloaders_dict[phase]:

                    if self.is_use_cuda:
                        inputs_1, single_image_label_scans, single_image_label_sizes, single_image_label_types, \
                        single_image_label_emphas = inputs_1.cuda(), single_image_label_scans.cuda(), \
                    ...
                    running_loss0 += loss0.item() * inputs_1.size(0)
                    running_loss1 += loss1.item() * inputs_1.size(0)
                    running_loss2 += loss2.item() * inputs_1.size(0)
                    running_loss3 += loss3.item() * inputs_1.size(0)
